...
std::unordered_set<std::string> ItemGenerator::getUsedNames() const { return used_names; }
...
ItemType ItemGenerator::randomType()
{
    ...
    if (roll < 0.33) return WEAPON;
    if (roll < 0.66) return ACCESSORY;
    return ARMOR;
}
...
std::string ItemGenerator::generateUniqueName(ItemType type)
{
    ...
    for (size_t i = 0; i < ATTEMPTS; i++) {
        ...
        switch (type) {
        case WEAPON:   bank = &weapon_types;    break;
        case ARMOR:    bank = &armor_types;     break;
        case ACCESSORY:bank = &accessory_types; break;
        default: break;
        }
        if (bank) name += randomChoice(*bank) + " ";
...
ItemGenerator::ItemGenerator(std::uint32_t seed) : rng(seed) {}
...
std::unordered_set<Item> ItemGenerator::getRandomItems(size_t count)
{
    ...
    for (size_t i = 0; i < count; i++) generated.insert(randomItem());
    return generated;
}
